...
Programs used: Python (\pkg{statsmodels}); R (\pkg{lavaan}, \pkg{semTools}); SPSS Amos 23 for replication
...
We defer further analysis of the data to \cref{sec:data_analysis}, and use the log-transformed data in the initial replication, following \cite{panHowIndustrializationTrade2019}.
...
Energy intensity, per-capita GDP, and number of patents are plotted on a logarithmic scale, as we are interested in growth rates.
Trade openness and industrial share are fractions, and plotted on a linear scale; over the range of these two variables, applying a logistic transformation yields a visually indistinguishable result.
...
\item The scales of the data sometimes differ by orders of magnitude between countries, so pooling data will require additional transformation.
\end{itemize}
