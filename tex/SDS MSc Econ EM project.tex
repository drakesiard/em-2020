...
\noindent 
Based on the data available and the analysis above, we can only replicate two of the four hypotheses: H.1 and H.4.
...
Another measure of this discrepancy is SRMR, which at 0.041 is below the cut-off value of 0.08, indicating that at least by one metric the covariance residuals may not be excessively large; however, it is clear that the model fit is not ideal.
...
For the same set of variables, we update the data set to the most recent 
