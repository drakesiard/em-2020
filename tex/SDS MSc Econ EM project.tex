...
Running a Levin-Lin-Chu test across all the series in an individual country as a panel yields a similar result: for Bangladesh, the p-value of the test on the log data is $\approx0.32$, whereas that on the log-differenced data is $<0.01$, confirming the more imprecise results above.
...
\cref{tab:normality_test} shows the results of a Jarque-Bera test, and we cannot reject the null hypothesis of normality in a majority of cases.

The most striking fact about the log differenced data is that much of the correlation structure is no longer significant, suggesting the possibility that the previous results were the result of spurious regression between I(1) series.
...
Running the regression from 1986 onward, and imputing the missing EIb and P\_GDPb data, yields a similar result; running the regression on the non-PPP-adjusted data yields a significant $P_{EI,P\_GDP}$ coefficient, whereas $P_{EIb,P\_GDPb}$ in the regression above is not significant, suggesting that the correlation does indeed come from the currency conversion. 

None of the fit statistics (shown in \cref{tab:updated_fit_statistics}) necessarily indicate a bad fit; however, with such a weak correlation structure, they have little diagnostic power. Inspecting the correlation residuals in \cref{fig:updated_fit_residuals}, there are still major outliers associated with the TI variable.

Running the regression on India, a comparable country with a longer energy intensity timeseries (from a different source, but in a similar unit: EIc) and a stronger correlation structure (\cref{fig:logdiff_correlations_india})
...
\begin{figure}[htbp]
\begin{subfigure}{\textwidth}
\small{\input{./plots/model.logdiff.fit.summary.tex}}
\caption{Model summary}
\end{subfigure}
...
\begin{figure}[htbp]\ContinuedFloat
\begin{subfigure}{\textwidth}
\input{./plots/model.logdiff.fit.estimates.tex}
\caption{Parameter estimates}
\end{subfigure}
\caption{Updated path model output}
\end{figure}
