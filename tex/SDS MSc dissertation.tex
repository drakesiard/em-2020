...
If leave aside for a moment the time-heterogeneity, and assume common time-specific effects, we can use one of the most commonly applied dynamic panel data models, the GMM estimator most often cited as \cite{arellanoTestsSpecificationPanel1991}\footnote{
Though, as pointed out in \cite{fritschGMMEstimationLinear2019}, originally proposed in \cite{holtz-eakinEstimatingVectorAutoregressions1988}.} and often referred to as difference GMM.

Following the notation of \cite{fritschGMMEstimationLinear2019}, the basic construction of the linear dynamic panel data model can be described as:
\begin{align}
...
\end{align}
where $i$ and $t$ identify individuals and time periods, $y$ is the endogenous variable with up to $p$ lags, $\boldsymbol{x}$ are explanatory variables with up to $q$ lags, and the error term $u$ is composed of an individual-specific effect $a_i$, a time-specific effect $\lambda_t$ and an observation-specific  idiosyncratic remainder $\varepsilon_{i,t}$.

The difference GMM estimator acquires its name via its method of dealing with the individual-specific effect $a_i$ by differencing \cref{eq:panel}.

\cite{blundellInitialConditionsMoment1998} and is known as the system GMM.
...
In addition, the specification tests for over-identifying restrictions is more likely to fail to reject the null of validity.
...
A good package: \pkg{pdynmc} \cite{fritschPdynmcPackageEstimating2019}
