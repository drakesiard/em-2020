...
\cite{shahbazDynamicLinksEnergy2013} introduce another variable, the provision of domestic credit to the private sector, as a proxy for financial development, separating integration into the global financial system from integration into world trading structures. Limiting their investigation to China and using an ARDL bounds testing approach, they also find bidirectional causality and a significant positive relationship between trade openness and energy consumption per capita; however, also they do not investigate energy intensity.
...
The add financial openness as measured by the Chinn-Ito index of \citep{chinnWhatMattersFinancial2006}, and apply some more comprehensive diagnostics. 
...
\begin{itemize}
\item technological innovation, as measured by total patent (resident and non-resident) application count, following \cite{panHowIndustrializationTrade2019}.
...
\begin{table}[htbp]
\centering
\input{./plots/dis/diffGMM_subset_reg_result_main.tex}
\caption{Difference-GMM regression results, restricted panel}
\label{tab:diffgmm_coeff_subset_main}
\end{table}
\renewcommand{\arraystretch}{1}

Starting with the original model TS, we see that at a first glance all the coefficients agree in sign with our initial hypotheses: energy intensity appears follow the Kuznets curve (positive linear relationship, negative quadratic relationship) with respect to affluence and has the expected negative linear relationship with urbanisation, population density, and trade.
...
Adding the technological innovation indicator $TI$ and the financial openness indicator $KAOPEN$ gives the model TS+TI+KO, whose coefficients yield an unexpected result: in the new specification, while the coefficients of the original regression maintain their sign and gain in significance, the coefficient of $TS$ is now negative (and just as remarkably, becomes significant).
...
Substituting $CTS$ for $TS$ (the CTS model) yields an unexpected result: the sign of the other regressors remains the same, and their significance is confirmed, but $CTS$ itself has a positive (though insignificant) coefficient. 
...
Applying the specification and identification tests described in \cref{sec:methodology} to all four models, we see that there is strong evidence that at least one of the slope cofficients and at least one of the time dummies are non-zero, as p-values of the F-tests for both are well below 0.01 and the null hypotheses of joint zero coefficients are rejected in both cases.
...
However, it is worth noting that the test ``depends on the assumption that $N$ is large. Large has no precise definition, but applying it to panels with $N = 20$, for instance, seems worrisome" \cite{roodmanHowXtabond2Introduction2009}.
In our case, $N = 22$, well within the range of worry.
...
\begin{table}[htbp]
\centering
\input{./plots/dis/diffGMM_full_reg_result_main.tex}
...
} can be found in \cref{sec:graph_appendix}.
...
\noindent
All of the Python and R code and output are incorporated into Jupyter Notebooks, which have also been converted to HTML for ease of examination. For example, the FX analysis described in \cref{sec:data} can be found here:
...
We examine plots of the residuals versus the fitted values, for the subset panel in \cref{fig:gmm_residual_plot_subset} and \cref{fig:gmm_residual_plot_all}
...
\centering
\resizebox{\textwidth}{!}{%
\input{./plots/dis/diffGMM_subset_reg_result.tex}
}
\caption{Difference-GMM regression results, restricted panel}
...
\centering
\resizebox{\textwidth}{!}{%
\input{./plots/dis/diffGMM_full_reg_result.tex}
}
\caption{Difference-GMM regression results, all countries}
