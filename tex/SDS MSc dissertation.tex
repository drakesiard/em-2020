...
\end{itemize}
Our hypotheses for this model are:
\begin{itemize}
\item Energy intensity will decrease as population density increases. 
\item Energy intensity will increase with affluence up to a certain point, and then decrease (the EKC hypothesis).
...
We incrementally test each of our additions to the with the final EKC-type Model IV of \cite{rafiqUrbanizationOpennessEmissions2016} by running a difference GMM with two lags of the exogenous variable on the original model (with the original trade share $TS$, so for shorthand we designate this model TS).
...
financial openness measure $KAOPEN$ of \citep{chinnWhatMattersFinancial2006} and denote these models CTS+TI and CTS+TI+KAOPEN, respectively.
\cref{tab:diffgmm_coeff_subset} shows the results of these regressions are 
